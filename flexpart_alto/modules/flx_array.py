...
def ds_add_ll_dis(ds):
    ds2 = ds
    la_dis = (ds2[co.LAT] - co.ORI_LAT)
    lo_dis = (ds2[co.LON] - co.ORI_LON)
...
def add_chc_lpb(ax):
    ax.scatter(co.CHC_LON_, co.CHC_LAT_, marker='.', color='b',
...
def get_r_dis(ds, lat_center=co.ORI_LAT, lon_center=co.ORI_LON):
...
def get_th_ang(ds, lat_center=co.ORI_LAT, lon_center=co.ORI_LON):
...
def data_array_to_logpolar(da: xr.DataArray,
                           r_round_log: float,
                           th_round_rad: float,
                           lat_center: float = co.ORI_LAT,
                           lon_center: float = co.ORI_LON,
    ...
    #todo: this statement is the
    # most time consuming so far it is the loading
    # of the xarray that takes the most time
    ...
    #todo: 2021-03-21_11-59-41_ here was the previous error, we just changed this
    r_th_da[co.LON] = r_th_to_lon(lon_center, r_th_da[co.R_CENTER],
                                  r_th_da[co.TH_CENTER])
...
def trim_swap_dim_coord(nds: xr.Dataset, hours: int):
    ...
    time_h = xr.zeros_like(nds1[co.TIME], dtype=int) + tr
...
def add_lat_lon_corners(ds):
    ...
    for v in val_list:
        ds[v[0]] = r_th_to_lat(co.ORI_LAT, v[2], v[3])
        ds[v[1]] = r_th_to_lon(co.ORI_LON, v[2], v[3])
        ds = ds.set_coords(v[0])
        ds = ds.set_coords(v[1])
    return ds


compressed_netcdf_save = za.compressed_netcdf_save
