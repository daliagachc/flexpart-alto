...
RELEASE_TEMP = '''  {start_dt_rel}       ID1, IT1        beginning date and time of release
  ...
  {lo_left}            XPOINT1 (real)  longitude [deg] of lower left corner
  {la_bottom}            YPOINT1 (real)  latitude [deg] of lower left corner
  {lo_right}            XPOINT2 (real)  longitude [deg] of upper right corner
  {la_top}            YPOINT2 (real)  latitude [DEG] of upper right corner
...
def format_release_template(release_dict=None) -> str:
...
def create_dic_range(*,
    ...
    for k, v in base_dict.items():
        join_dict[k] = v
    dt_index: pd.DatetimeIndex = pd.date_range(d_start, d_end,
                                               freq='{}H'.format(h_frq))
    ...
    df[rel_dic_for_key] = df.apply(
        lambda r: format_release_dict(r[rel_dic_un_key]), axis=1)
    df[RELEASE_DIC_STR_KEY] = df[rel_dic_for_key].apply(
        lambda v: format_release_template(v))
    return df
...
def create_all_avail_files(d1, d2, doms00=None,
    ...
    if doms00 is None:
        doms00 = ['01', '02', '03', '04']
    for d in doms00:
        create_avail_file(d1, d2, d, frq_file, frq_sim, output_dir,
                          file_base_name)
