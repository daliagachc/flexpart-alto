...
import os
import flexpart_alto.modules.daily_back.daily_back as db
import pandas as pd
import subprocess
...
class ConfigDayRun:
    ...
    def get_dt_last_release(self) -> None:
        dt_rel = dt.datetime(*self.DATE_SIMULATION.timetuple()[:6])
        dt_rel = dt_rel + dt.timedelta(
            hours=self.NUMBER_OF_RELEASES - self.RELEASE_FRQ)
    ...
    def create_available_paths(self) -> None:
        ...
        for d in self.DOMAINS:
            ...
            path_local = os.path.join(self.DAY_PATH,
                                      self.AVAILABLE_FILE_BASE_NAME + d)

            avail_paths.append(path)
            avail_paths_loc.append(path_local)
        self.AVAILABLE_PATHS = avail_paths
        self.AVAILABLE_PATHS_LOCAL = avail_paths_loc
    ...
    def fill_out_run_flx(self):
        ...
        fill_dic = dict(
            ...
            RUN_BASE_NAME=self.RUN_BASE_NAME,
            DAY_STR=self.DAY_STR,
            WRFOUT_PATH = self.WRFOUT_PATH,
            FLX_PATH_OUT = self.FLX_PATH_OUT,
            DAY_PATH_TAITO = self.DAY_PATH_TAITO
        )
    ...
    def get_class_dict(self) -> dict:
        ...
        for i in dir(self):
            try:
                ...
                # 2021-03-11_13-36-29_, removed by diego
                # log.ger.error(error)
                print(error)
                pass
        dic = {}
    ...
    def __init__(self, init_dic: dict = None):
        if init_dic is None:
            print('dict empty')
        else:
    ...
    def create_download_file(self):
        # todo here we create the string to download the files
        import flexpart_alto.modules.bucket_download as bdown
        from pathlib import Path
        df = bdown.get_download_file_list(self.AVAILABLE_PATHS_LOCAL)
        bdown.save_download_script(
            dir_out='./data_in',
            df=df,
            bucket=self.BUCKET_IN,
            path_script=Path(self.DAY_PATH) / 'download_files.sh'
        )
        pass
