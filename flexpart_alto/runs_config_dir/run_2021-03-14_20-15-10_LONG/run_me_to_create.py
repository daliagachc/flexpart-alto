...
# %%
run_base_name = 'run_2021-03-14_20-15-10_LONG'
...
import os
import pathlib
all_runs = glob.glob(str(pack_path / 'runs_config_dir' / run_base_name / "*/run_flex.sh"))

df = pd.Series(all_runs)


def _f(r):
    p =  pathlib.Path(r)
    rr = os.path.join(*p.parts[-2:])
    rr = f'sbatch ./{rr}'
    return rr

df = df.apply(_f)

df.to_csv('multi_sbatch_run.sh',index=False,header=False)
# %%